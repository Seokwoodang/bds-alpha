...
test('PA5 · 임대 물건 등록 → 임대수익률·계약 만기·보유세 표시', async ({ page }) => {
  await login(page);
  await page.getByRole('button', { name: '+ 자산 추가' }).click();
  await page.getByLabel('단지명/별칭').fill('지산임대');
  await page.getByLabel('지역').selectOption('성동구');
  await page.getByLabel('매물 유형').selectOption('지식산업센터');
  await page.getByLabel('전용면적 (㎡)').fill('50');
  await page.getByLabel('매입가 (만원)').fill('100000'); // 10억
  await page.getByLabel('매입일').fill('2024-01-10');
  // 임대 토글(라벨 클릭) → 임대 필드 노출
  await page.getByText('임대(월세) 주는 물건', { exact: false }).click();
  await page.getByLabel('월세 (만원)').fill('300');
  await page.getByLabel('월세 수령일 (1~31)').fill('25');
  await page.getByLabel('계약 만기일').fill('2026-08-15');
  await page.getByRole('button', { name: '추가' }).click();

  await expect(page.getByText(/임대수익률/)).toBeVisible();
  ...
  page.on('dialog', (d) => d.accept());
  await page.getByRole('button', { name: '삭제' }).first().click();
  await expect(page.getByText('지산임대')).toHaveCount(0);
});
